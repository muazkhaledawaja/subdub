...
export const sendReminders = serve(async (context) => {
    ...
    for (const reminder of reminders) {
        ...
        if (now.isBefore(reminderDate)) {
            console.log(
                `Sleeping until next reminder: ${reminder.label
...
const fetchSubscription = async (cxt, subscriptionId) => {
    return await cxt.run('get subscription', () => {
        return Subscription.findById(subscriptionId).populate('user', 'name email');
    })
}

const sleepUntilReminder = async (cxt, label, date) => {
    console.log(`Workflow ${label} sleeping until ${date.toISOString()}`);
    await cxt.waitUntil(label, date.toDate());
}

const triggerReminder = async (cxt, label) => {
    return await cxt.run(label, async () => {
        console.log(`Triggering reminder: ${label}`);
        await sendReminderEmail({
            to: subscription.user.email,
            type: label.replace('Reminder-', '').replace('-days before', ' days before reminder'),
